...
В основном, при необходимости, хаскель вычисляет выражения до состояния слабой-нормальной...
а вовсе не до дна. Это повышает степень ленивости и снижает неопределенность (шанс выполнить расходящуюся ветку).
```hs
-- WHNF
...
\x -> x -- нет, NF
3 -- нет, NF
(,) undefined -- да, частично примененный конструктор
...
-- первое выражение вычисляется, но не конца
